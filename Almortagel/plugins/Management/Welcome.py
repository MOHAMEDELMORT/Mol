...
def welcomepic(pic, user, chatname, id, uname):
    background = Image.open("Almortagel /assets/dil.png")
    pfp = Image.open(pic).convert("RGBA")
    pfp = circle(pfp)
    pfp = pfp.resize((1157, 1158))
    draw = ImageDraw.Draw(background)
    font = ImageFont.truetype('Almortagel /assets/font.ttf', size=110)
    welcome_font = ImageFont.truetype('Almortagel /assets/font.ttf', size=60)
...
async def greet_group(_, member: ChatMemberUpdated):
    ...
    try:
        ...
        temp.MELCOW[f"welcome-{member.chat.id}"] = await app.send_photo(
            ...
            photo=welcomeimg,
            caption=f"""
نـورت في {member.chat.title}
➖➖➖➖➖➖➖➖➖➖➖➖
اسـمك ✧ {user.mention}
ايـديك ✧ {user.id}
يـوزرك ✧ @{user.username}
➖➖➖➖➖➖➖➖➖➖➖➖
""",
